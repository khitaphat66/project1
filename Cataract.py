...
while True:
    ...
    if key == ord('c'):
        ...
        # แปลงเป็นขาวดำเพื่อตรวจจับดวงตา
        ...
        if len(eyes) == 0:
            print("ไม่พบดวงตา ลองใหม่อีกครั้ง")
            continue

        # เลือกดวงตาอันแรกที่เจอ
        (x, y, w, h) = eyes[0]

        # ขยายขอบเขตตัดรูปนิดหน่อย
        margin = 20
        x1 = max(0, x - margin)
        y1 = max(0, y - margin)
        x2 = min(capture.shape[1], x + w + margin)
        y2 = min(capture.shape[0], y + h + margin)
